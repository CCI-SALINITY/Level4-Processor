function nfil = merge_SSS_OPER_003_004(conf_file)
...
% Ice Descriptor
% absolute correction
% date : 11/2020 (CCI+SSS year 3 project)
% 1/ SMAP filtering with WSmax and RRmaxSMAP
% 2/ add representatitivity errors :
%        <rep50km_50km key="rep50km_50km">F:\vergely\SMOS\CCI\livrables\CCI_soft_year3\aux_files\ERR_REP\ERR_REP_1d50km_7d50km_mr2_ctm.mat<rep50km_50km>
%        <rep50km_50km_2 key="rep50km_50km_2">F:\vergely\SMOS\CCI\livrables\CCI_soft_year3\aux_files\ERR_REP\ERR_REP_1d50km_30d50km_mr2_ctm.mat<rep50km_50km_2>
%        <rep50km_50km_3 key="rep50km_50km_3">F:\vergely\SMOS\CCI\livrables\CCI_soft_year3\aux_files\ERR_REP\ERR_REP_7d50km_30d50km_mr2_ctm.mat<rep50km_50km_3>
%        <rep150km_50km key="rep150km_50km">F:\vergely\SMOS\CCI\livrables\CCI_soft_year3\aux_files\ERR_REP\ERR_REP_1d150km_7d50km_mr2_ctm.mat</rep150km_50km>
%        <rep150km_50km_2 key="rep150km_50km_2">F:\vergely\SMOS\CCI\livrables\CCI_soft_year3\aux_files\ERR_REP\ERR_REP_1d150km_30d50km_mr2_ctm.mat</rep150km_50km_2>
% 3/ Error propagation : direct matrix inversion whithout slipping window for
% monthly SSS error. For weekly error, no change (truncation works well
% because inter-dwell biases are freezed).
% 4/ mean ISAS for SSS prior with alldata=2
% 5/ ajout d'une boucle sur les plans SMOS
...
alldata=conf.param.alldata.CONTENT;      % ==1 all dwells are taken for SSS reference computation ==2, use mean(ISAS)
...
err_repr_smap=conf.param.err_repr_smap.CONTENT;            % ==1 for applying representativity error on Aquarius
err_repr_smos=conf.param.err_repr_smos.CONTENT;            % ==1 for applying representativity error on Aquarius

variability_hebd=conf.param.variability_hebd.CONTENT;            % ==1 for applying representativity error between monthly and weekly SSS fields
RRmaxSMAP=conf.param.RRmaxSMAP.CONTENT;

hebdo_factor=conf.param.hebdo_factor.CONTENT; % facteur a appliquer sur la variabilité pour le passage mensuel->hebdo ajout year2
...
repres150km_7d='not used';
if err_repr_aqua==1
    repres150km_7d=conf.AUX_FILE.rep150km_50km.CONTENT;
    if exist(repres150km_7d)
        fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [I] AUX File   : ' repres150km_7d 10]);
    else
        fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [W] AUX File   : ' repres150km_7d ' NOT FOUND' 10]);
        fclose(ftrace);
        return
    end
    load(repres150km_7d);
    errrepres150km_7d=errrepres;
else
    fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [W] AUX File repres150km_7d NOT USED' 10]);
end

% year3
repres150km_30d='not used';
if err_repr_aqua==1
    repres150km_30d=conf.AUX_FILE.rep150km_50km_2.CONTENT;
    if exist(repres150km_30d)
        fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [I] AUX File   : ' repres150km_30d 10]);
    else
        fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [W] AUX File   : ' repres150km_30d ' NOT FOUND' 10]);
        fclose(ftrace);
        return
    end
    load(repres150km_30d);
    errrepres150km_30d=errrepres;
else
    fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [W] AUX File repres150km_30d NOT USED' 10]);
end

% year3
repres50km_7d='not used';
if err_repr_aqua==1
    repres50km_7d=conf.AUX_FILE.rep50km_50km.CONTENT;
    if exist(repres50km_7d)
        fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [I] AUX File   : ' repres50km_7d 10]);
    else
        fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [W] AUX File   : ' repres50km_7d ' NOT FOUND' 10]);
        fclose(ftrace);
        return
    end
    load(repres50km_7d);
    errrepres50km_7d=errrepres;
else
    fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [W] AUX File repres50km_7d NOT USED' 10]);
end

% year3 : representativity 50km 30d : rep50km_50km_2
repres50km_30d='not used';
if err_repr_aqua==1
    repres50km_30d=conf.AUX_FILE.rep50km_50km_2.CONTENT;
    if exist(repres50km_30d)
        fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [I] AUX File   : ' repres50km_30d 10]);
    else
        fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [W] AUX File   : ' repres50km_30d ' NOT FOUND' 10]);
        fclose(ftrace);
        return
    end
    load(repres50km_30d);
    errrepres50km_30d=errrepres;
else
    fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [W] AUX File repres50km_30d NOT USED' 10]);
end

% year 3 : variability 7d : rep50km_50km_3
rep50km_50km='not used';
if variability_hebd==1
    rep50km_50km=conf.AUX_FILE.rep50km_50km_3.CONTENT;
    if exist(rep50km_50km)
        fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [I] AUX File   : ' rep50km_50km 10]);
    else
        fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [W] AUX File   : ' rep50km_50km ' NOT FOUND' 10]);
        fclose(ftrace);
        return
    end
    load(rep50km_50km);
    variability_7d=hebdo_factor*errrepres;  % ajout year2
else
    fwrite(ftrace,[datestr(now,'yyyy-mm-dd HH:MM:SS.FFF') ' ' nom_proc1 ': [W] AUX File rep50km_50km NOT USED' 10]);
end

conf.repres150km_7d=repres150km_7d;
conf.repres150km_30d=repres150km_30d;
conf.repres50km_7d=repres50km_7d;
conf.repres50km_30d=repres50km_30d;
conf.rep50km_50km=rep50km_50km;
...
conf.variability_hebd=variability_hebd;
...
% pour aquarius niveau 3 : distinction asc-desc seulement
...
for iorb=1:2  % boucle asc-desc
            ...
            for imonth0=1:nmonth
                ...
                for iday=1:31
                    ...
                    if exist([dirdatasmap namemat '.mat'])~=0
                        ...
                        % eSSSsel1=0.45./(0.015.*SSTsel1+0.25);      % year 1 pas d'erreur dans SMAP RSS. Bruit correspondant à une erreur de 0.45K
                        error_factor_smap_sel=error_factor_smap(indlonsel,indlatsel);
                        eSSSsel1=error_factor_smap_sel.*0.38./(0.015.*SSTsel1+0.25);        % year 2
                        % filtrage WS
                        WSsel1=WS1(indlonsel,indlatsel);                    % year 3
                        rainsel1=rain1(indlonsel,indlatsel);                % year 3
                        indWS=find(WSsel1>WSmax | rainsel1>RRmaxSMAP);      % year 3
                        SSSsel1(indWS)=NaN;                                 % year 3
                        % afte
                        SSSsel2=SSS2(indlonsel,indlatsel);
                        tSSSsel2=tSSS2(indlonsel,indlatsel);
                        SSTsel2=SST2(indlonsel,indlatsel);
                        % eSSSsel2=0.45./(0.015.*SSTsel2+0.25);      % year 1 pas d'erreur dans SMAP RSS. Bruit correspondant à une erreur de 0.45K
                        eSSSsel2=error_factor_smap_sel.*0.38./(0.015.*SSTsel2+0.25);
                        % filtrage WS
                        WSsel2=WS2(indlonsel,indlatsel);                    % year 3
                        rainsel2=rain2(indlonsel,indlatsel);                % year 3
                        indWS=find(WSsel2>WSmax | rainsel2>RRmaxSMAP);      % year 3
                        SSSsel2(indWS)=NaN;                                 % year 3
                        ...
                        for igp=1:(nlonregion*nlatregion)
                            ...
                            if isnan(SSS1)==0;
                                ...
                                data(igp,idw).capt(nSn)=capt;       % ajout year3
                            end
            ...
            for imonth0=1:nmonth
                ...
                for iday=1:31
                    ...
                    if exist([dirdataaqua namemat '.mat'])~=0
                        ...
                        load([dirdataaqua namemat])   % 'SSS1','tSSS1','SST1',eSSS1
                        ...
                        for igp=1:(nlonregion*nlatregion)
                            ...
                            % ibeam=beamsel1(igp);
                            if isnan(SSS1)==0;
                                idw=idwellaqua;   % beam  % deux beams virtuels pour les niveau 3
                                nSn=nS(idw,igp)+1;
                                nS(idw,igp)=nSn;
                                data(igp,idw).SSS(nSn)=SSS1;
                                data(igp,idw).eSSS(nSn)=eSSS1;
            ...
            for imonth0=1:nmonth
                ...
                for iday=1:31
                    ...
                    if exist([dirdatasmos namemat '.mat'])~=0
                        ...
                        load([dirdatasmos namemat])   % 'SSScorrRR','eSSS0','tSSS0','chiSSS0','idwSSS0','SST0', 'Acard','nplan'
                        
                        for iplan=1:nplan
                            SSSsel1=SSScorrRR(indlonsel,indlatsel,iplan);
                            tSSSsel1=tSSS0(indlonsel,indlatsel,iplan);
                            eSSSsel1=eSSS0(indlonsel,indlatsel,iplan);
                            idwSSSsel1=idwSSS0(indlonsel,indlatsel,iplan);
                            WSsel1=WS0(indlonsel,indlatsel,iplan);                      % ajout year2
                            chi2Psel1=chiSSS0(indlonsel,indlatsel,iplan);               % il s'agit du chi, pas du chi2
                            SSTsel1=SST0(indlonsel,indlatsel,iplan);
                            Acardsel1=Acard0(indlonsel,indlatsel,iplan);
                            Acard_modsel1=Acard0_mod(indlonsel,indlatsel,iplan);        % ajout year2
                            Dg_icesel1=0.*Acard_modsel1; %Dg_Suspect_ice0(indlonsel,indlatsel); % ajout year2 -> on met à 0 en year 3 (detection par Acard pour SMOS)
                            flag_outsel1=flag_many_outlier(indlonsel,indlatsel,iplan);  % ajout year2
                            ...
                            for igp=1:(nlonregion*nlatregion)
                                ...
                                if isnan(SSS1)==0 & idw <= maxidw & idw >= minidw & chi<3. & eSSS1<3. & Acard1>42. & SSS1>2. & SSS1<45. & WS1<WSmax & dAcard1<2 & flag_out1<0.5;  % year2
                                    ...
                                    data(igp,idw).tSSS(nSn)=tSSS1;
                                    data(igp,idw).SST(nSn)=SST1;
                                    data(igp,idw).Dg_ice1(nSn)=Dg_ice1;  % plus utilisee en year3 (detection ice par Acard)
                                    data(igp,idw).capt(nSn)=capt;    % ajout year3
                                end
                            end
                        end
    ...
    clear SSS eSSS
    % if iad==1  % on ne charge isas qu'a la premiere boucle
    %     load('isas');
    % end
end
...
% attention, cela redistribue les numéros de dwell dans data
data=data(:,idsel);
nS=nS(idsel,:);
...
if err_repr_aqua==1
    errrepr150km_7d=errrepres150km_7d(indlonsel,indlatsel,:);
    errrepr150km_7d=reshape(errrepr150km_7d,nlonregion*nlatregion,12);
    errrepr150km_30d=errrepres150km_30d(indlonsel,indlatsel,:);
    errrepr150km_30d=reshape(errrepr150km_30d,nlonregion*nlatregion,12);
end

if aqua_ok==1
    idw_aqua=[idw_aqua_A idw_aqua_D];
    ngp=size(data,1);
    for idw=1:length(idw_aqua)
        inds=find(idsel==idw_aqua(idw));
        for ii=1:ngp
            if length(data(ii,inds).SSS)>0
                if err_repr_aqua==1
                    err150km_7d=squeeze(errrepr150km_7d(ii,:));
                    tSSS=data(ii,inds).tSSS;
                    dvec=datevec(tSSS);
                    monthaqua=dvec(:,2);
                    data(ii,inds).eSSS_7d=err150km_7d(monthaqua);  % ajout year 3
                    err150km_30d=squeeze(errrepr150km_30d(ii,:));
                    data(ii,inds).eSSS_30d=err150km_30d(monthaqua);
                else  % on met les erreurs de representativite a 0
                    data(ii,inds).eSSS_7d=0.*data(ii,inds).eSSS;
                    data(ii,inds).eSSS_30d=0.*data(ii,inds).eSSS;
                end
            end
        end
    end
...
% year 3
if err_repr_smap==1 | err_repr_smos==1
    errrepr50km_7d=errrepres50km_7d(indlonsel,indlatsel,:);
    errrepr50km_7d=reshape(errrepr50km_7d,nlonregion*nlatregion,12);
    errrepr50km_30d=errrepres50km_30d(indlonsel,indlatsel,:);
    errrepr50km_30d=reshape(errrepr50km_30d,nlonregion*nlatregion,12);
end

% year 3 err repres SMAP
if err_repr_smap==1 & smap_ok==1
    idw_smap=[idw_smap_A idw_smap_D];
    for idw=1:length(idw_smap)
        inds=find(idsel==idw_smap(idw));
        ngp=size(data,1);
        for ii=1:ngp
            if length(data(ii,inds).SSS)>0
                if err_repr_smap==1
                    err50km_7d=squeeze(errrepr50km_7d(ii,:));
                    tSSS=data(ii,inds).tSSS;
                    dvec=datevec(tSSS);
                    monthsmap=dvec(:,2);
                    data(ii,inds).eSSS_7d=err50km_7d(monthsmap);  % ajout year 3
                    err50km_30d=squeeze(errrepr50km_30d(ii,:));
                    data(ii,inds).eSSS_30d=err50km_30d(monthsmap);
                else  % on met les erreurs de representativite a 0
                    data(ii,inds).eSSS_7d=0.*data(ii,inds).eSSS;
                    data(ii,inds).eSSS_30d=0.*data(ii,inds).eSSS;
                end
...
% year 3 err repres SMOS
if smos_ok==1
    idw_smos=[idw_smos_A idw_smos_D];
    for idw=1:length(idw_smos)
        inds=find(idsel==idw_smos(idw));
        ngp=size(data,1);
        for ii=1:ngp
            if length(data(ii,inds).SSS)>0
                if err_repr_smos==1
                    ...
                    dvec=datevec(tSSS);
                    monthsmos=dvec(:,2);
                    data(ii,inds).eSSS_7d=err50km_7d(monthsmos);  % ajout year 3
                    err50km_30d=squeeze(errrepr50km_30d(ii,:));
                    data(ii,inds).eSSS_30d=err50km_30d(monthsmos);
                else % on met les erreurs de representativite a 0
                    data(ii,inds).eSSS_7d=0.*data(ii,inds).eSSS;
                    data(ii,inds).eSSS_30d=0.*data(ii,inds).eSSS;
                end
...
% variabilite hebdo
if variability_hebd==1
    stdhebd=variability_7d(indlonsel,indlatsel,:);
...
ttmois=tt(1:12)+0.5;  % en adequation avec le calcul des mois pour GCmVAR
...
tx1=cos(2*pi*ttmois/12);
ty1=sin(2*pi*ttmois/12);
...
vec_varmens=invH*stdsel';  % stdest=Cm0*vec_varmens;

% year 3 erreur representativite smos SMAP
if err_repr_smap==1 | err_repr_smos==1
    vec_repr50km_7d=invH*errrepr50km_7d';
    vec_repr50km_30d=invH*errrepr50km_30d';
end
% year 3 erreur representativite aquarius
if err_repr_aqua==1
    vec_repr150km_7d=invH*errrepr150km_7d';
    vec_repr150km_30d=invH*errrepr150km_30d';
end


% pour la variabilite hebdomadaire a partir de la variabilite
% MERCATOR 7 jours <-> 30 jours
vec_varhebd=invH*stdhebd';
...
GCmVAR=sigpri*exp(-(txx1-txx2).*(txx1-txx2)/xi_cov/xi_cov-(tyy1-tyy2).*(tyy1-tyy2)/xi_cov/xi_cov);
        ...
        if NSSS(igp) > 100
            ...
            for idw=1:ndwell
                ...
                if length(SSS0) > 0
                    ...
                    eSSS_7d=[eSSS_7d data(igp,idw).eSSS_7d];
                    eSSS_30d=[eSSS_30d data(igp,idw).eSSS_30d];
                    capt=[capt data(igp,idw).capt];
                end
            end
            ...
            % coefficient a appliquer sur les biais pour tenir compte des
            % effets de la SST
            coefSST=1+0.015*deltaSST./(-0.015.*SSTGP-0.25);
            
            GG1=eye(ndata,ndata);
            % pour le calcul du prior, on prend la dwell centrale ou toute
            % les donnees ou ISAS si alldata=2
            SSSref=[];
            if smos_ok==1
                if alldata==0
                    SSSref=data(igp,idwref).SSS;
                elseif alldata==2
                    % il faut tenir compte des periodes sans donnees,
                    % notamment aux hautes latitude
                    isasTS=squeeze(isasSSSsel(:,ilo0,ila0));
                    %  isasTS=isasSSSsel(:,50,50);
                    SSSref=interp1(datenumisas,isasTS,tSSSGP,'linear',NaN);
                else
                    SSSref=SSSGP;
                end
            end
            ...
            if length(SSSref) > 1
                SSSmed=nanmedian(SSSref);
            else
                SSSmed=SSSref;
            end
            SSSprior = tSSSGP.*0+SSSmed;  % sur les donnees
            ...
            % preparation de la covariance a priori sur la base journaliere
            % tt(ntime)
            % interpolation de la variabilite mensuelle sur tt
            vec3=vec_varmens(:,igp);
            stdtt_day_m=GCmVAR*vec3;
            [st1 st2]=meshgrid(stdtt_day_m,stdtt_day_m);
            st0c=(st1.*st2);
            matvartm1=matvartm.*st0c;
            
            % preparation des erreurs de representativite
            if err_repr_smap==1 | err_repr_smos==1
                vec5=vec_repr50km_7d(:,igp);
                err_day_50km_7d=GCmVAR*vec5;
                err_day_50km_7d=err_day_50km_7d(indtime);
                vec5=vec_repr50km_30d(:,igp);
                err_day_50km_30d=GCmVAR*vec5;
                err_day_50km_30d=err_day_50km_30d(indtime);
            end
            
            if err_repr_aqua==1
                vec5=vec_repr150km_7d(:,igp);
                err_day_150km_7d=GCmVAR*vec5;
                err_day_150km_7d=err_day_150km_7d(indtime);
                vec5=vec_repr150km_30d(:,igp);
                err_day_150km_30d=GCmVAR*vec5;
                err_day_150km_30d=err_day_150km_30d(indtime);
            end
            
            % deuxieme facon de calculer l'erreur de representativite
            eSSS_30d_2=SSSGP*0;
            eSSS_7d_2=SSSGP*0;
            for icapt=1:3
                ind=find(capt==icapt);
                if icapt==1 & err_repr_smap==1               % smap
                    eSSS_7d_2(ind)=err_day_50km_7d(ind);
                    eSSS_30d_2(ind)=err_day_50km_30d(ind);
                elseif  icapt==3 & err_repr_smos==1          % smos
                    eSSS_7d_2(ind)=err_day_50km_7d(ind);
                    eSSS_30d_2(ind)=err_day_50km_30d(ind);
                elseif icapt==2 & err_repr_aqua==1              % aquarius
                    eSSS_7d_2(ind)=err_day_150km_7d(ind);
                    eSSS_30d_2(ind)=err_day_150km_30d(ind);
                end
            end
            
            % figure; hold on; plot(tSSSGP,stdtt_data,'.'); plot(tt,stdtt_day_m); hold off
            % ajout de l'erreur de representativite
            eSSSGP2=eSSSGP.*eSSSGP;
            eSSSGP2=eSSSGP2+eSSS_30d_2.*eSSS_30d_2;
            ...
            for it=1:ntime
                ...
                if nSSS >0
                    ...
                    % CpGG1=CpGG(it,indt);
                    % invmat=CpGG1*inv(GCmGCd1)*CpGG1';
                    % supression year 3
                    % sigSSS_mens(it)=sqrt(stdtt_day_m(it).*stdtt_day_m(it)-invmat(1,1));
                    ...
                    icecont(it)=0;
                    % supression year 3
                    % sigSSS_mens(it)=stdtt_day_m(it);
                end
                % pour PCTVAR
                signatm(it)=stdtt_day_m(it);
            end
            %%%%% year3
            Cpost=stdtt_day_m.^2-diag(CpGG*inv(GCmGCd)*CpGG');
            sigSSS_mens=sqrt(Cpost);
            
            %%%% year3 pour vérifier le nouveau calcul d'erreur
%                         eSSSGP2=eSSSGP.*eSSSGP;
%                         eSSSGPt=sqrt(eSSSGP2+eSSS_30d_2.*eSSS_30d_2);
%                         figure; subplot(2,2,1); hold on; title(['month SSS error, lon=' num2str(lon(indlonsel(ilo0)),2) ',lat=' num2str(lat(indlatsel(ila0)),2)]);
%                         plot(tSSSGP,eSSSGP,'.');
%                         plot(tSSSGP,eSSSGPt,'r.'); plot(tt,stdtt_day_m,'k-');
%                         plot(tt,sigSSS_mens,'g-');  xlabel('time'); ylabel('SSS error'); grid on;
%                         legend('SSS L2 error','SSSerr+repr','SSS L4 a priori error','SSS L4 error'); hold off;
%                         subplot(2,2,2); hold on; title('nout'); plot(tt,nout);  xlabel('time'); ylabel('noutlier'); grid on; hold off;
%                         subplot(2,2,3); hold on; title('SSS'); plot(tt,SSSsol_3sigma); plot(tSSSGP,SSSGP,'.'); xlabel('time'); ylabel('SSS');
%                         if alldata==2
%                             plot(tSSSGP,SSSref,'g.')
%                         end
%                         grid on; hold off;
%                         %  subplot(2,2,4); hold on; title('eSSS'); plot(tSSSGP,eSSSGPt,'.'); plot(tSSSGP,eSSS_30d,'r.'); plot(tSSSGP,eSSS_30d_2,'r.'); xlabel('time'); ylabel('eSSS'); grid on; hold off;
%                         if err_repr_aqua==0
%                             saveas(gcf,['error_month_lon_' sprintf('%d',round(10*lon(indlonsel(ilo0)))) '_lat_' num2str(10*lat(indlatsel(ila0)),2)],'fig')
%                         else
%                             saveas(gcf,['error_month_lon_' sprintf('%d',round(10*lon(indlonsel(ilo0))))  '_lat_' num2str(10*lat(indlatsel(ila0)),2) '_withREP_isas'],'fig')
%                         end
%                         
%                         keyboard
            ...
            if switch_hebdo==1
                ...
                eSSSGP2=eSSSGP2+eSSS_7d_2.*eSSS_7d_2;
                ...
                % corrige les donnees)
                vec4=vec_varhebd(:,igp);
                stdtt_day_h=GCmVAR*vec4;
                ...
                % solution lissée en ajoutant la variabilite a l'erreur
                ...
                %%%%% year3 : inutile de faire du global pour
                %%%%% l'hebdo car le biais est gelé et la
                %%%%% troncature n'affecte pas le calcul.
                % Cpost=stdtt_day.^2-diag(CpGG*inv(GCmGCd)*CpGG');
                % sigSSS_mens=sqrt(Cpost);
                % GCmGCd_no_outlier=stdtt_day_h.^2-diag(CpGG*inv(Cp1_h+diag(eSSSGP2))*CpGG');
                % errtot=sqrt(sigSSS_hebd.^2+sigSSS_mens.^2);
                
                %%%% year3 pour vérifier le nouveau calcul d'erreur
                % reinitialisation de l'erreur
                %                 eSSSGP2=eSSSGP.*eSSSGP;
                %                 eSSSGPt=sqrt(eSSSGP2+eSSS_7d_2.*eSSS_7d_2);
                %                 figure; subplot(2,2,1); hold on; title(['weekly SSS error, lon=' num2str(lon(indlonsel(ilo0)),2) ',lat=' num2str(lat(indlatsel(ila0)),2)]);
                %                 plot(tSSSGP,eSSSGP,'.');
                %                 plot(tSSSGP,eSSSGPt,'r.');
                %                 plot(tt,stdtt_day_h,'k-'); plot(tt,real(sigSSS_hebd),'g-');
                %                 xlabel('time'); ylabel('SSS error'); grid on;
                %                 legend('SSS L2 error','SSSerr+repr','SSS L4 a priori error','SSS L4 error'); hold off;
                %                 subplot(2,2,2); hold on; title('nout'); plot(tt,nout);  xlabel('time'); ylabel('noutlier'); grid on; hold off;
                %                 subplot(2,2,3); hold on; title('SSS'); plot(tt,SSSwindow); plot(tSSSGP,SSSGP,'.'); xlabel('time'); ylabel('SSS');
                %                 if alldata==2
                %                     plot(tSSSGP,SSSref,'g.')
                %                 end
                %                 grid on; hold off;
                %
                %                 if err_repr_aqua==0
                %                     saveas(gcf,['error_weekly_lon_' sprintf('%d',round(10*lon(indlonsel(ilo0)))) '_lat_' num2str(10*lat(indlatsel(ila0)),2)],'fig')
                %                 else
                %                     saveas(gcf,['error_weekly_lon_' sprintf('%d',round(10*lon(indlonsel(ilo0)))) '_lat_' num2str(10*lat(indlatsel(ila0)),2) '_withREP_isas'],'fig')
                %                 end
...
for ilo0=1:nilo
    for ila0=1:nila
        ...
        if isnan(bb(1))==0
            ...
            esmos(ilo0,ila0)=sol(1);   % si négatif alors prendre la médiane
