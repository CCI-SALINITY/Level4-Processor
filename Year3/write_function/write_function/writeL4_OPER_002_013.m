% function nfil=writeL4_OPER_002_013(conf_file)
...
% (attention : matlab utilise l'ordre inverse)
% ajout ancillary pour les key variables
% 26/04/2021 : correction flags lsc. Mise à 0 des flags ice entre +/-45
...
iquant=conf0.param.iquant.CONTENT;   % 5 pour la mï¿½diane, 8 pour le quantile ï¿½ 80%
...
nslice=300;   % nombre de jours à traiter par tranche
...
if nday0 > nslice
    ...
    inds=1:nslice:nday0;       % indices de dï¿½but et de fin de tranche
...
if update_netcdf==0    % on crï¿½e le fichier, on ne le met pas ï¿½ jour
    for itim=1:nday0
        ...
        Value= 'http://cci.esa.int/salinity - DOI:XXXXX';    % year2 DOI ï¿½ complï¿½ter
...
for intr=1:ntr
    ...
    for izone=1:nzone
        ...
        elseif no_quant==2 % on module avec la variabilite (ATBD year 1)
            ...
            iq=round((1.5*errtotsel-0.4)*10);  % numï¿½ro du quantile qui est echantillonne tous les 10%
            indlow=find(errtotsel<=0.6);
            iq(indlow)=5;   % correspond au quantile ï¿½ 50 %
            indhigh=find(errtotsel>=0.8);
            iq(indhigh)=8;   % correspond au quantile ï¿½ 80 %
    ...
    % on ajoute la contrainte nok0 a maskSSS pour n'avoir à gérer qu'un
    % seul mask global
    for iti=1:nday;
    ...
    for iti=1:nday;
        ...
        masksel=mask(1:size(corrSMOS0,1),:)-1;   % correction 26/04/2021 : le mask_smos en entree est = 1 si ocean (convention inverse CCI)
        ind_coast=find(dminsel<distmax | isnan(masksel));   % mask contient l'ancien masque SSS de la v1.8 et la v2.3 year3
        corrSMOS11(:,:)=zeros(size(corrSMOS0,1),nlat);  % 0 est pris comme good data (changements v1.5 -> v1.6)
...
for itim=1:nday0
    ...
    if exist(nameAcard)>0   % pas de produits si pas de masque Acard
       ...
       % mask1=zeros(length(lon_fixgrid),length(lat_fixgrid),1)-1;  % d'abord sur l'ensemble de la carte initialisation du flag ice a -1
        mask1=zeros(length(lon_fixgrid),length(lat_fixgrid),1);   % correction 26/04/2021 : on met 0 entre +/-45
        maskSSS0=zeros(length(lon_fixgrid),length(lat_fixgrid),1);
